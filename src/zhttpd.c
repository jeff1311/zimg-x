...
zimg_headers_conf_t * conf_get_headers(const char *hdr_str)
{
	...
	LOG_PRINT(LOG_DEBUG, "conf_get_headers: hdr_str = %s", hdr_str);
    if(hdr_str == NULL)
        return NULL;
...
{
	//liangjixun add 20160224
	LOG_PRINT(LOG_DEBUG, "=============binary_parse start=============");
    int err_no = 0;
...
/**
 * @brief get_crossdomain_cb The callback function of a POST request to upload a image.
 *
...
void post_request_cb(evhtp_request_t *req, void *arg)
{
    LOG_PRINT(LOG_DEBUG, "============post_request_cb() start============");
    int post_size = 0;
    char *buff = NULL;
    int err_no = 0;
    int ret_json = 1;
    ...
    LOG_PRINT(LOG_DEBUG, "Content-Type = %s", content_type);
    if(!content_type)
    {
